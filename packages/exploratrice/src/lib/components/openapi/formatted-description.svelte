<script lang="ts">
  ...
  const segments = $derived(
    hasSpecialTags(description)
      ? parseDescription(description)
      : [{ type: "text" as const, content: description }],
  )
</script>
  ...
  <p class={className}>
    {#each segments as segment}
      ...
      {:else if segment.type === "deprecated"}
        <Badge variant="destructive" class="mx-1 text-xs">Déprécié</Badge>
      {:else if segment.type === "example"}
        <Badge variant="outline" class="mx-1 text-xs">Exemple</Badge>
      {/if}
    {/each}
  </p>
{/if}
