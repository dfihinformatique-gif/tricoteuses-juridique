...
const loadDocumentPageInfos = async (
  ...
  const enrichedDocumentDir = documentDir.replace(
    "/Documents/",
    "/Documents_enrichis/",
  )
  let useEnriched = await fs.pathExists(enrichedDocumentDir)
  let currentDocumentDir = useEnriched ? enrichedDocumentDir : documentDir
  let documentFilename = useEnriched
    ? documentFileInfos.filename.replace(/\.html$/, "_avec_liens.html")
    : documentFileInfos.filename
  let documentPath = path.join(currentDocumentDir, documentFilename)
  if (!(await fs.pathExists(documentPath))) {
    useEnriched = false
    currentDocumentDir = documentDir
    documentFilename = documentFileInfos.filename
    documentPath = path.join(currentDocumentDir, documentFilename)
  }
  ...
  if (await fs.pathExists(documentPath)) {
    ...
    if (documentSegmentation !== undefined) {
      ...
      let offset = 0
      for (const { line, originalTransformation, type } of walkTableOfContents(
      ...
      >) {
        ...
        const replacementStart = originalTransformation.position.start + offset
    ...
    if (transformation && (await fs.pathExists(documentTransformationPath))) {
      ...
      if (documentSegmentation !== undefined) {
        ...
        documentTransformation = {
          ...
          title: "Simplification du HTML et ajout des ancres pour le sommaire",
        }
      }
    }
