...
<svelte:head>
  <title>{m.assemblee_documents_list_page_title()}</title>
</svelte:head>
...
<div class="container mx-auto max-w-7xl px-4 py-8">
  <PageBreadcrumb
    segments={[
      ...
      { label: m.assemblee_documents_list_breadcrumb_documents() },
    ]}
  />
  ...
  <div class="mb-8">
    <h1 class="mb-4 flex items-center gap-3 text-4xl font-bold">
      ...
      {m.assemblee_documents_list_heading()}
    </h1>
    <p class="text-lg text-muted-foreground">
      {m.assemblee_documents_list_description()}
    </p>
  </div>
