<script lang="ts">
  ...
  const documentsDiffPageInfos = $derived(
    await queryDocumentsDiffPageInfos([params.previousUid, params.uid]),
  )
...
{#if current === undefined}
  <PageBreadcrumb
    segments={[
      ...
      { label: `${m.assemblee_document_menu_trigger()} ${params.previousUid}` },
    ]}
  />
...
{:else if previous === undefined}
  ...
  <Alert.Root class="mx-auto w-fit max-w-xl" variant="destructive">
    <AlertCircleIcon />
    <Alert.Title
      >{m.assemblee_document_not_found({
        uid: params.previousUid,
      })}</Alert.Title
    >
  </Alert.Root>
