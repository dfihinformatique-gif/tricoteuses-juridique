	...
	try {
		...
		switch (service.id) {
			case "api-canutes-assemblee": {
				// Fetch OpenAPI spec
				const openApiResponse = await fetch("https://db.code4code.eu/canutes_assemblee/")
			...
			case "api-canutes-legifrance": {
				// Fetch OpenAPI spec
				const openApiResponse = await fetch("https://db.code4code.eu/canutes_legifrance/")
				openApiSpec = (await openApiResponse.json()) as OpenAPIV2.Document
			...
			case "api-parlement": {
				...
				openApiSpec = convertOpenAPI3to2(openApiSpec3, "https://parlement.tricoteuses.fr/")
